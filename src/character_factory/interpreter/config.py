...
A local model loads 4-bit quantized (nf4, bf16 compute) on CUDA: the
download is the full-precision weights, quantized as they stream in, and
the generation path stays inside about 10 GB so a card also driving a
desktop and other programs still fits. A CPU device runs full precision.
...
take precedence over the file and describe the default
...
def _section_defaults(section: dict) -> dict:
    ...
    sets its own."""
    return {key: section.get(key) for key in ("instruction",)}
